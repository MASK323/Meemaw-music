...
Meemaw Music 是一款基于 PySide6 开发的 Windows 桌面音乐播放器，界面整体参考网易云音乐客户端与黑胶唱片播放器的设计风格，支持榜单、歌单、歌词、评论、喜欢同步，并通过 QQ 扫码登录后同步会员信息、签到领取会员并匹配网络音源进行播放。
...
- 播放默认走 Chromium WebEngine 浏览器音频后端（HTMLAudioElement），与 MoeKoe 一致的播放路径，播放更稳定流畅
...
- 登录方式改为 QQ 扫码登录，登录后自动同步会员信息，并支持签到领取会员
...
- 账号页提供“签到领取会员”入口，登录后即可签到获取会员
- 签到成功后自动刷新会员状态与到期时间，今日签到状态实时展示
- 同一账号每天可签到一次，重复签到会提示“今天已经签到过了”
